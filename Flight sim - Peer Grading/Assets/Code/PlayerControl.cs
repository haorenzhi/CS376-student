...
public class PlayerControl : MonoBehaviour {
    ...
    private void FixedUpdate()
    {
        ...
        playerRB.AddRelativeForce(transform.forward * thrust);
        ...
        var collisions = Physics.OverlapSphereNonAlloc(transform.position,1, col, LayerMask.GetMask("Updrafts"));
        if (collisions > 0)
        {
            var comp = col[0].GetComponent<Updraft>();
            vrel = comp.WindVelocity - playerRB.velocity;
        } 


        float vf = Vector3.Dot(transform.forward, vrel);
        float vup = Vector3.Dot(transform.up, vrel);

        Vector3 liftval = LiftCoefficient * vf * vf * transform.up;

        Vector3 forwarddrag = Mathf.Sign(vf) * ForwardDragCoefficient * vf * vf * transform.forward;



        Vector3 updrag = Mathf.Sign(vup) * VerticalDragCoefficient * vup * vup * transform.up;

        Vector3 totaldrag = forwarddrag + updrag + liftval;

        playerRB.AddForce(totaldrag);
    ...
    {
        var landingplat = collision.gameObject.GetComponent< LandingPlatform>();
        

        if ((collision.gameObject.name == "LandingPlatform") && (playerRB.velocity.magnitude < landingplat.MaxLandingSpeed)) //if landingplat is null
